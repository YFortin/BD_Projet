    ...
    <v-app>
        <v-content>
            <v-container grid-list-md>
                ...
                <h1 class="display-1 black--text text-xs-center">
                    <span class="font-weight-bold">Top memes!!</span>
                </h1>
                <v-layout align-space-around justify-center row wrap fill-height>
                    <v-flex xs8 align-self-center>
                        <v-card color="#FAFAFA" flat v-if="topmemes">

                            <h3 class="headline font-weight-bold mb-2">Category: {{ topmemes[carouselIndex].category
                                }}</h3>
                            <v-card-title primary-title>
                                <h3 class="headline font-weight-bold mb-2">Title: {{ topmemes[carouselIndex].title }}</h3>
                            </v-card-title>
                            <v-carousel
                                    ...
                                    hide-controls
                            >
                                <v-carousel-item contain v-for="(item,i) in topmemes" :key="i"
                                                 :src="item.url"></v-carousel-item>
                            </v-carousel>
                            <v-card-text v-for="(comment, i) in topmemes[carouselIndex].comments" :key="i"> {{ comment.text
                                }} --
                                {{comment.user_name}}
                                <v-divider></v-divider>
                            </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            const response = await MemberAPI.Memes.getUnseenMemes();
            const secondResponse = await MemberAPI.Memes.topmemes();
            this.items = [];
            this.topmemes = [];
            secondResponse.data.top_memes.forEach(meme => {
                this.topmemes.push(meme);
            });
            response.data.unseen_memes.forEach(meme => {
                this.items.push(meme);
            });
